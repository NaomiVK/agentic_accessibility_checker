...
class DecisionAgent {
    constructor() {
        this.complexIssues = scan_config_1.SCAN_CONFIG.decisionThresholds.complexIssuesForClaudeAnalysis;
        this.criticalThreshold = scan_config_1.SCAN_CONFIG.decisionThresholds.criticalViolationsLimit;
        this.incompleteThreshold = scan_config_1.SCAN_CONFIG.decisionThresholds.incompleteResultsLimit;
    }
    /**
     * Analyzes scan results and categorizes pages based on violation types and complexity
     */
    categorizePages(scanResults) {
    ...
    makeDecision(scanResult) {
        ...
        if (criticalViolations.length >= this.criticalThreshold) {
            return {
                ...
                complexViolations: this.getComplexViolations(violations),
     ...
     */
    needsHumanAnalysis(violations, incomplete) {
        ...
        return violations.some(violation => visualVerificationRules.some(rule => violation.id.includes(rule)));
    }
    /**
     ...
     */
    getComplexViolations(violations) {
        return violations.filter(violation => this.complexIssues.some(complexIssue => violation.id.includes(complexIssue) || violation.tags.includes(complexIssue)));
    }
    /**
     * Builds a detailed reason for why Claude analysis is needed
     */
    buildClaudeAnalysisReason(violations, incomplete, complexViolations) {
        const reasons = [];
    ...
    calculatePriority(violations) {
        ...
        if (criticalCount > 0)
            return 'Critical';
        if (seriousCount >= 3 || (seriousCount >= 1 && moderateCount >= 5))
            return 'High';
        if (seriousCount >= 1 || moderateCount >= 3)
            return 'Medium';
        return 'Low';
    }
    /**
     ...
     */
    logDecisionMetrics(result, duration) {
        const metrics = {
        ...
        console.log('METRICS: Decision Agent Results:', Object.assign(Object.assign({}, metrics), { passRate: `${((metrics.passedPages / metrics.totalPages) * 100).toFixed(1)}%`, claudeAnalysisRate: `${((metrics.claudeNeededPages / metrics.totalPages) * 100).toFixed(1)}%`, criticalRate: `${((metrics.criticalPages / metrics.totalPages) * 100).toFixed(1)}%`, avgDecisionTime: `${(duration / metrics.totalPages).toFixed(2)}ms per page` }));
    }
    /**
     * Gets detailed breakdown of decision categories for a set of pages
     */
    getDecisionBreakdown(scanResults) {
     ...
     */
    getHighPriorityPages(scanResults) {
     ...
     */
    getBatchProcessingOrder(scanResults) {
