...
export class EnhancedDecisionAgent {
   ...
   * Analyze violations with contextual understanding
   */
  private analyzeViolationsInContext(violations: ViolationResult[], incomplete: ViolationResult[]): { factors: ReasoningFactor[] } {
  ...
  private detectComplexPatterns(violations: ViolationResult[]): string[] {
    ...
    const focusViolations = violations.filter(v => v.id.includes('focus') || v.id.includes('keyboard'));
    ...
    const semanticViolations = violations.filter(v => v.id.includes('semantic') || v.id.includes('role'));
    ...
    const formRelated = violations.filter(v => 
    ...
    // Navigation pattern
    const navRelated = violations.filter(v => 
      v.id.includes('landmark') || v.id.includes('heading') || v.id.includes('bypass') ||
  ...
  private analyzeViolationRelationships(violations: ViolationResult[]): string[] {
    ...
    if (hasColorContrast && hasFocusVisible) {
      relationships.push('Color contrast and focus visibility issues may share root cause in design system');
    }
    ...
    if (hasKeyboardIssues && !hasSkipIssues) {
      relationships.push('Keyboard navigation issues without proper skip mechanisms compounds barriers');
    }
   ...
   */
  private applyHeuristicReasoning(violations: ViolationResult[], _incomplete: ViolationResult[]): { factors: ReasoningFactor[] } {
    const factors: ReasoningFactor[] = [];
    ...
    const foundationalIssues = violations.filter(v => 
      v.id.includes('html-has-lang') || v.id.includes('document-title') || 
      v.id.includes('landmark') || v.id.includes('heading-order')
    );
    ...
    const interactiveViolations = violations.filter(v => 
      v.nodes.some(n => 
        n.html.includes('button') || n.html.includes('link') || 
        n.html.includes('input') || n.html.includes('select')
      )
    );
    ...
    if (interactiveViolations.length / violations.length > 0.6) {
      factors.push({
        type: 'heuristic',
        description: 'Majority of violations on interactive elements indicates UX accessibility gaps',
        weight: 0.65,
        evidence: [`${interactiveViolations.length} of ${violations.length} violations affect interactive elements`]
      });
    }
   ...
   */
  private calculateDecisionScores(factors: ReasoningFactor[], violations: ViolationResult[], _incomplete: ViolationResult[]): Map<keyof typeof DECISION_CATEGORIES, number> {
   ...
   */
  private selectBestDecision(scores: Map<keyof typeof DECISION_CATEGORIES, number>): { decision: keyof typeof DECISION_CATEGORIES; confidence: number } {
   ...
   */
  private getAlternativeDecisions(scores: Map<keyof typeof DECISION_CATEGORIES, number>, primaryDecision: keyof typeof DECISION_CATEGORIES): AlternativeDecision[] {
   ...
   */
  private generateAlternativeRationale(decision: keyof typeof DECISION_CATEGORIES, score: number): string {
    const rationales: Record<keyof typeof DECISION_CATEGORIES, string> = {
      ...
      CLAUDE_NEEDED: 'Complex patterns suggest human-level analysis would provide valuable insights',
   ...
   */
  private generateContextualAnalysis(violations: ViolationResult[], factors: ReasoningFactor[]): string {
   ...
   */
  private extractLearningInsights(url: string, violations: ViolationResult[], decision: { decision: keyof typeof DECISION_CATEGORIES; confidence: number }): string[] {
   ...
   */
  private updateContextualPatterns(url: string, violations: ViolationResult[], decision: string): void {
  ...
  private identifyPageType(url: string, violations: ViolationResult[]): string {
    ...
    if (urlPath.includes('form') || violations.some(v => v.nodes.some(n => n.html.includes('<form')))) {
      return 'form-page';
    ...
    } else if (urlPath.includes('article') || urlPath.includes('blog') || urlPath.includes('post')) {
      return 'content-page';
    }
   ...
   */
  private generateAdaptiveSuggestions(violations: ViolationResult[], reasoning: DecisionReasoning): string[] {
    ...
    if (autoFixableViolations.length > 0) {
      suggestions.push(`${autoFixableViolations.length} violations can be auto-remediated with simple fixes`);
    }
    
    // Suggest design system improvements
    const designSystemIssues = violations.filter(v => 
      v.nodes.length > 10 && (v.id.includes('color') || v.id.includes('focus'))
    );
    ...
    if (reasoning.confidence < 0.7) {
      suggestions.push('Add automated accessibility tests to catch these issues earlier in development');
    }
   ...
   */
  private identifyUncertaintyFactors(violations: ViolationResult[], incomplete: ViolationResult[], reasoning: DecisionReasoning): string[] {
    ...
    if (incomplete.length > violations.length * 0.5) {
      factors.push('High ratio of incomplete to complete violations suggests dynamic content complexity');
    }
  ...
  private identifyViolationClusters(violations: ViolationResult[]): string[] {
    ...
    const formCluster = violations.filter(v => 
      v.id.includes('label') || v.id.includes('input') || v.id.includes('form')
    ).length;
    
    if (formCluster >= 3) clusters.push('form accessibility');
    
    const navigationCluster = violations.filter(v => 
      v.id.includes('landmark') || v.id.includes('heading') || v.id.includes('skip')
    ).length;
    
    if (navigationCluster >= 3) clusters.push('navigation structure');
    
    const interactiveCluster = violations.filter(v => 
      v.id.includes('focus') || v.id.includes('keyboard') || v.id.includes('click')
    ).length;
  ...
  private generateEnhancedReason(decision: { decision: keyof typeof DECISION_CATEGORIES; confidence: number }, factors: ReasoningFactor[]): string {
  ...
  private calculateEnhancedPriority(violations: ViolationResult[], reasoning: DecisionReasoning): 'Low' | 'Medium' | 'High' | 'Critical' {
    const criticalCount = violations.filter(v => v.impact === 'critical').length;
    const seriousCount = violations.filter(v => v.impact === 'serious').length;
  ...
  private estimateEffortWithLearning(violations: ViolationResult[], historicalInsights: { factors: ReasoningFactor[] }): string {
    const baseEstimate = this.calculateBaseEffort(violations);
    
    // Adjust based on historical patterns
    const hasRecurringIssues = historicalInsights.factors.some(f => 
      f.type === 'historical' && f.description.includes('recurring')
    );
  ...
  private createErrorDecision(url: string, error: string): EnhancedPageDecision {
    ...
    return {
      ...
      adaptiveSuggestions: ['Investigate scan failure cause', 'Consider manual accessibility review'],
  ...
  private analyzeHistoricalPatterns(url: string, _violations: ViolationResult[]): { factors: ReasoningFactor[] } {
    const factors: ReasoningFactor[] = [];
    const domain = new URL(url).hostname;
   ...
   */
  public provideFeedback(url: string, actualCategory: keyof typeof DECISION_CATEGORIES, feedback: string): void {
    const domain = new URL(url).hostname;
    const decisions = this.decisionHistory.get(domain) || [];
  ...
  public exportLearningInsights(): {
    ...
    const domainPatterns = Array.from(this.learningMemory.patternHistory.entries()).map(([domain, patterns]) => ({
      domain,
      patterns
    }));
    
    const decisionAccuracy = Array.from(this.learningMemory.decisionOutcomes.entries()).map(([key, outcomes]) => {
