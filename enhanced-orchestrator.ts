...
class EnhancedAccessibilityOrchestrator {
  ...
  private decisionAgent: EnhancedDecisionAgent;
  ...
  constructor(options: any, urlsConfig?: UrlsConfig) {
    ...
    this.decisionAgent = new EnhancedDecisionAgent();
  ...
  private setupSubscriptions(): void {
    ...
    this.communicationHub.subscribe('insight', message => {
      ...
      if (payload.type === 'learning' || (payload.confidence && payload.confidence > 0.8)) {
        this.metrics.insights.push(
          `${message.fromAgent}: ${payload.description || 'No description'}`
        );
      }
    });
    ...
    this.communicationHub.subscribe('coordination', message => {
      ...
      console.log(
        `[COORDINATION] ${message.fromAgent} requests ${payload.action || 'unknown action'}`
      );
    });
  }
  ...
  private async performIntelligentScanning(urls: string[]): Promise<PageScanResult[]> {
    ...
    const prioritizedUrls =
      (responseMessage?.payload as { prioritized?: string[] })?.prioritized || urls;
  ...
  private async makeEnhancedDecisions(scanResults: PageScanResult[]): Promise<any> {
    ...
    console.log(
      `- Claude Analysis Needed: ${categorized.claudeNeeded.length} (Avg confidence: ${this.calculateAvgConfidence(categorized.claudeNeeded)})`
    );
  ...
  private async performAdaptiveAnalysis(decisionData: any): Promise<AnalysisResult[]> {
    ...
    const analysisRequests = categorized.claudeNeeded.map((decision: any) => {
      ...
      return {
        ...
        pageContext: {
          ...
          hasForm:
            scanResult?.violations.some((v: ViolationResult) =>
              v.nodes.some((n: ViolationNode) => n.html.includes('<form'))
            ) || false,
          hasNavigation:
            scanResult?.violations.some((v: ViolationResult) =>
              v.nodes.some((n: ViolationNode) => n.html.includes('nav'))
            ) || false,
          hasModal:
            scanResult?.violations.some((v: ViolationResult) =>
              v.nodes.some(
                (n: ViolationNode) => n.html.includes('modal') || n.html.includes('dialog')
              )
            ) || false,
        },
    ...
    this.metrics.agentMetrics.set('claude-analyst', {
      ...
      criticalFindings: results.filter(r => r.remediationSteps.some(s => s.priority === 'Critical'))
        .length,
    });
  ...
  private async generateEnhancedReports(
    decisionData: any,
    claudeResults: AnalysisResult[]
  ): Promise<void> {
    const phaseStart = Date.now();
    console.log('\n=== Phase 4: Comprehensive Reporting with Insights ===');

    const { decisions, scanResults } = decisionData;
  ...
  private generateOptimizationRecommendations(insights: any[], messageAnalysis: any): void {
    ...
    if (highConfidenceInsights.length > 0) {
      this.metrics.recommendations.push(
        ...highConfidenceInsights.filter(i => i.suggestedActions).flatMap(i => i.suggestedActions)
      );
    }
  ...
  private async generateInsightsReport(
    decisions: any[],
    claudeResults: AnalysisResult[]
  ): Promise<void> {
    const insightsReport = {
      ...
      decisionInsights: {
        ...
        lowConfidencePages: decisions
          .filter(d => d.reasoning.confidence < 0.6)
          .map(d => ({
            url: d.url,
  ...
  private calculateAvgConfidence(decisions: any[]): string {
    ...
    const avgConfidence =
      decisions.reduce((sum, d) => sum + (d.reasoning?.confidence || d.confidence || 0), 0) /
      decisions.length;
